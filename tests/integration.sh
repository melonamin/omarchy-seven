...
status=$(omarchy-shell dots status 2>/dev/null) || fail "Dots service is not answering (is the plugin enabled?)"
...
for n in 1 2 3 4 5 6 7; do
  omarchy-shell dots clear "$n" >/dev/null
done
sleep 1
status=$(omarchy-shell dots status)
[[ $(json_get filled <<<"$status") == 0 ]] || fail "clear did not empty every dot"
pass "clear empties a dot"

# --- append and persistence ---------------------------------------------------

[[ $(omarchy-shell dots append 4 "first line") == ok ]] || fail "append reported failure"
[[ $(omarchy-shell dots append 4 "second line") == ok ]] || fail "second append reported failure"
sleep 1

read_back=$(omarchy-shell dots read 4)
...
target=$(omarchy-shell dots capture "no dot number given")
...
for bad in 0 8 x ""; do
  out=$(omarchy-shell dots read "$bad" 2>&1 || true)
...
for _ in 1 2 3 4 5; do
  sleep 0.4
  [[ $(omarchy-shell dots read 6) == "edited outside the panel" ]] && break
done
[[ $(omarchy-shell dots read 6) == "edited outside the panel" ]] \
